...
def drawLine(screen,points,brush_stats,usp):
    ...
    if len(points['x']) < 2: 
        cv2.circle(screen, (points['x'][-1],points['y'][-1]) , brush_stats['size'] //2 ,brush_stats['color'], -1)
    ...
    if usp: 
        ...
        if distance > usp_sensitivity:
            cv2.circle(screen, (points['x'][-1],points['y'][-1]) , brush_stats['size'] //2 ,brush_stats['color'], -1) # !!!!!!!!!!!! //2
            return

    cv2.line(screen, initial_point, final_point, brush_stats['color'], brush_stats['size'])
...
def draw(img, mask,copypaint,copyimg,centroids,brush_stats,usp,switch,draw_history):
    if switch:
        f = open("demofile2.txt", "a")
        for drawing in draw_history:
            f.write(str(drawing))
            drawLine(copypaint if switch == 0 else copyimg,drawing,brush_stats,usp)
    ...
    #mark with red x
    cv2.drawMarker(img, cccentroid, color=[0, 0, 255], thickness=3,markerType= cv2.MARKER_TILTED_CROSS, line_type=cv2.LINE_AA,markerSize=25)
    # recording centroids
    if not cccentroid:
        return
    ...
    if len(centroids['x']) >= 5 :
        ...
        drawLine(copypaint if switch == 0 else copyimg,centroids,brush_stats,usp)
        if switch:
            draw_history.append(centroids)
...
#
        ...
        copyimg = deepcopy(img)
        draw(img,mask,copypaint,copyimg,centroids,brush_stats,usp,switch,draw_history)
