...
def drawLine(copyimg,copypaint,points,brush_stats,usp,switch):
    ...
    if len(points['x']) < 2: 
        cv2.circle(copypaint if switch == 0 else copyimg, (points['x'][-1],points['y'][-1]) , brush_stats['size'] //2 ,brush_stats['color'], -1)
    ...
    if usp: 
        ...
        if distance > usp_sensitivity:
            cv2.circle(copypaint if switch == 0 else copyimg, (points['x'][-1],points['y'][-1]) , brush_stats['size'] //2 ,brush_stats['color'], -1) # !!!!!!!!!!!! //2
            return
